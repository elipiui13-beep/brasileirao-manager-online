...
function install(){
  ...
  window.mpSendChat=async function(){try{let i=document.getElementById('mpChatInput'),text=i?.value.trim();if(!text)return;const r=await api(`/api/rooms/${MP.code}/chat`,{method:'POST',body:JSON.stringify({text})});MP.chat=r.chat||[];if(i)i.value='';drawBar()}catch(e){alert(e.message)}};
